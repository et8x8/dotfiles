...
| 楽譜エディタ | 数字譜モデル編集、拍子表示／警告、フリーレイヤー | ローカル状態、保存時 Worker |
...
- `schemaVersion`: 整数。
- `parts[]`: パートごとのイベント列（数字・休符・奏法タグ・小節境界等）。MVP 記号集合は実装リポジトリで列挙し、未知コードは保存可・表示はグレーアウト可（ADR）。
...
**拍数**: 小節内の音符・休符の長さを、実装で定める単一アルゴリズム（付点・連桁・タイ等の扱いをここで固定）により拍単位に合算する。SPA と Worker は同一実装（共有モジュール）を参照する。
