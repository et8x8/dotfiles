...
- `schemaVersion`: 整数。
- `parts[]`: パートごとのイベント列（数字・**八分／十六分を含む音価**・**三連符グループ**・休符・奏法タグ・**小節境界と小節線**・**小節繰り返し記号**（各イベントは `repeatKind` を **`repeatOneBar` または `repeatTwoBars`** のみで持つ）・**装飾音（Spec「装飾音の一覧（MVP）」の各種別コード）**・**音楽記号（Spec「音楽記号の一覧（MVP）」の各種別コード）**等）。未知コードは保存可・表示はグレーアウト可（ADR）。
...
**拍数**: 小節内の音符・休符の長さを、単一アルゴリズム（付点・連桁・タイ、**八分・十六分**、**三連符**は Spec「三連符の拍数（MVP）」、**装飾音**は Spec「装飾音の拍寄与（MVP）」）により拍単位に合算する。SPA と Worker は同一実装（共有モジュール）を参照する。

## 反復展開規則（再生）

**正本**: `docs/spec/erhu-numeric-notation-webapp.md` の「小節繰り返しの再生（MVP）」。実装は同節の手順および `repeatKind` に従う。

- 小節繰り返し記号は `parts[]` 内のイベントとして保持する。
- 再生エンジンは Spec の手順に従い論理タイムラインを構築する。
- 印刷ビューは繰り返し記号の可視化を必須とし、展開譜の重ね表示の要否は UI オプションとする（ADR AC7）。
...
- バリデーション失敗（楽譜 JSON が [REQ-ERHU-022](../spec/erhu-numeric-notation-webapp.md#req-erhu-022) に違反する等）→ 400。
