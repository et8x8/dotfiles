...
- **1 ブランチで複数の Draft ADR を持ってよい**。設計内容・トピックごとにファイルを分ける (`guardrails.md` の行数・トークン分割に従い、1 ファイルが肥大化しないようにする)。
...
- `guardrails.md` の「設計ドキュメントの分割 (行数・トークン)」に従う。リポジトリに `AGENTS.md` / `CLAUDE.md` の上限があれば**それを優先**。
...
2. **ADR の作成 / 編集 / 削除**
   ...
   - 各 ADR に **`## Recommendations`** を書く (`guardrails.md` の「Draft ADR のレコメンド」)。Decision / 受け入れ条件に入れない改善提案 (例: Web アプリならロガー追加)。該当なしは `なし`。
3. **supersede 判定**
...
- `guardrails.md` の「成果物記述における前後工程参照の禁止」に反し、受け入れ条件や本文で Test・実装・ソースコードに詳細や妥当性を委ねること (ユーザーが明示した場合のみ例外)。
