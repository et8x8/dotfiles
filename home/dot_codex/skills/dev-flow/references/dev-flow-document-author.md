...
3. **`docs/adr/draft/dev-flow-state.json` の git 履歴** (PR 用ブランチ上の差分) を参照し、現 dev-flow の影響範囲を把握する。Done 完了後は本流で state は削除されるが、**PR 更新用ブランチでは工程ごとに更新され続ける**ため、履歴追跡に使える (工程判定の正本は引き続き `dev_flow_completed_through` のみ)。
