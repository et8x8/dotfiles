...
- [ ] ADR・用件定義・基本設計・Spec のいずれにも、`references/guardrails.md` の「成果物記述における前後工程参照の禁止」に反する記述 (後工程への委ね、後工程を唯一の定義源とする指示など) がないか (ユーザー明示の例外のみ許容)
- [ ] 各 Markdown が `references/guardrails.md` の「設計ドキュメントの分割 (行数・トークン)」に照らし**過大でないか** (500 行超・目安超なら分割漏れとして報告)
...
違反が無ければ親エージェントは工程 2 (`dev-flow-implementer`) への進行可否をユーザーに確認できる。
