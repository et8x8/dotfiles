...
- **Subagents (作業 / 補助 8 + 監査 4)**: `~/.cursor/agents/dev-flow-*.md` (フラット配置)
...
| --- | --- | --- | --- |
| `dev-flow` | 4 工程 (設計束 → TDD 周回 → ドキュメント生成 → 完了) をすべて扱う統合 skill。状態判定とフロー制御のみで、各工程の作業 / 補助 / 監査は subagent に委譲する | `dev-flow/SKILL.md` | **`true`** (プロンプトから明示的に呼ぶエントリポイント) |

進行ルールの要点 (詳細は `dev-flow/SKILL.md`):

- 親 (`dev-flow` skill) は状態判定・工程選択・subagent 起動・結果の受け取り・ユーザー報告のみを行い、各 subagent の成果物作成 / 編集 / 監査 / 品質コマンド実行 / 失敗分類 / Done 処理を代行しない。
- 次工程に進む明示指示が無ければ進まない。
...
| 2. 実装 (TDD) | `dev-flow-test-author` → `dev-flow-quality-runner` → `dev-flow-implementer` (`dev-flow-failure-triager` は失敗時) | `dev-flow-implementation-auditor` |
| 3. ドキュメント生成 | `dev-flow-document-author` | `dev-flow-document-auditor` |
| 4. 完了 (Done) | `dev-flow-done-runner` | (設計束〜工程 3 の auditor を再実行) |

各 subagent ファイルには YAML frontmatter (`name` / `description` / `model: inherit`) と本文 (役割 / 制約 / 入出力 / 手順 / テンプレート / やってはいけないこと / 完了報告 / 戻り先案内) を含む。`dev-flow` skill から Task ツールで spawn される。
...
| `dev-flow-*.md` (12 ファイル) | `~/.cursor/agents/` 直下 | 各工程の作業 / 補助 / 監査 subagent |
...
- **`readonly: true` の使い分け**:
  ...
  - Implementation auditor → 設定しない (テスト実行・リンタ実行のため。本文で「成果物編集禁止」を強く明示)
