...
2. 本 Skill (`4-dev-flow-fix`) の手順に厳密に従う。
...
**`docs/adr/draft/dev-flow-state.json` を必ず削除する** (Done 完了後の状態は「ファイルが無いこと」で表す)。あわせて `docs/adr/draft/` に Draft ADR が残っていないことを確認し、クリーンでない場合はユーザーに確認する。
