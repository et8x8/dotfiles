...
- **現在地の判定**では、存在すれば**まず state ファイルを Read** し、`dev_flow_phase` を取得する。その後 `git status` と変更ファイル種別で整合を取る。**無い場合**は `git status` と下表のみで推定する。
- 各工程が完了したら**次の工程**に合わせて `dev_flow_phase` と `last_updated` を更新する。**Done 工程 (commit) 完了後は state ファイルを必ず削除**する (状態は「ファイル無し」で表す)。
...
- **現在の工程**: 実装 2.1 (Test)
- **state**: docs/adr/draft/dev-flow-state.json (`dev_flow_phase: test`)
- **未コミット変更**: 14 ファイル
