...
2. 本 Skill (`5-update-implementation`) の手順に従い、テストを通すコードを実装させ、実装後に全テストの**成功**を確認させる。
...
4. 失敗が残る場合:
   - 実装の不備 → 実装を修正して再実行。
   - テストが間違っている (実装の都合で書き換えたい) → **禁止**。Spec に立ち返り、必要なら ADR から見直す。`3-update-spec` または `2-update-adr` に戻る。
...
すべてのテストが通ったら、`6-update-document` skill (`document-author`) で Document 工程に進む。
