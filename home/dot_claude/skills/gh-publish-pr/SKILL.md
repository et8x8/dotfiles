...
Immediately create a subagent and delegate the entire publishing workflow to that subagent. Keep the parent responsible for user communication and approvals.
...
- If HEAD is detached or checked out on the base branch, derive a concise slug from the changes and create a non-conflicting `claude/<slug>` branch.
